...
- 🚦 **Far fewer requests behind the scenes, so downloads keep the connection to themselves.** To show an app's real name, icon, languages and certificate before you install it, Omnify reads a small piece of each remote app file. It was asking GitHub for that piece in a way GitHub always refuses, then recovering in two more requests, and starting over from scratch for every single reader, version and app: twelve requests where five would do, a third of them certain to fail. All that traffic goes to the same place a download comes from, which is why a download started while a list was still settling could crawl while the same one moments later ran at full speed. Omnify now remembers what a site already told it.
...
- ⬇️ **An app installed from an external source is no longer offered an older version from the catalogue.** The version number Android actually compares is not the one you read: F-Droid rebuilds some apps with a numbering of its own, so Every Door's 6.0.0 from F-Droid counted as higher than the 7.1.0 published by the app's own developer, and the Updates tab offered it as an upgrade. Omnify now leaves an app's updates to whoever installed it, instead of comparing two numberings that were never meant to meet. Switching to the catalogue's build stays available from the app's own page, and the automatic installer follows the same rule, so it can no longer put one of these on your device by itself.
- 🌍 **An app's languages are read right even when its translation files are named after the app.** Several projects, Flutter's own tooling among them, name each translation file after the string set rather than the language, and that word took the language's place: an app with 43 translations listed "App (France)" and "App (DA)", and claimed not to be translated into French while listing French.
