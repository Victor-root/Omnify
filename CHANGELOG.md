...
### 🐛 Fixed
- 🖼️ **A source now shows the same icon before installing as after:** it was not showing a poorer copy of the app's icon, it was showing a different picture altogether. Since Android 8 an app's real icon is an "adaptive" one, two layers the system composes and masks itself; the flat image a project also keeps in its repository is only the fallback for Android 7 and below, is never what a modern phone draws, and is therefore routinely left behind at an older version of the artwork. Confirmed on Victor-root/OpenMessages, whose leftover flat image is a smaller, thinner, three-line speech bubble while the icon Android actually draws is a bold two-line one. Omnify now builds that same adaptive icon from the repository, layers, mask and all, and shows it from the moment a source is added. Sources already tracked pick it up on their next refresh. Below Android 8 nothing changes, because there the flat image genuinely is what the device uses. A repository with no adaptive icon, or one drawn in a way this can't reproduce, keeps the flat image rather than showing something half-drawn.
- 🖼️ **Icons scale more sharply**, and a "master" copy of an icon that some projects keep beside the real one to regenerate it from is no longer mistaken for the icon itself: it used to tie the real one for best candidate and lose only by the luck of file-listing order, which on another repository could have shown a near-blank image.
...
- 🔁 **The restore dialog no longer flashes closed and open again mid-restore:** restoring a backup that includes Settings applies its theme colour right away, and the app recreates its screen to pick that up properly. Doing so while the rest of the archive (repositories, external sources, …) was still being applied closed the dialog, then briefly reopened it in the new colour once the screen came back, tempting a second tap on "Restaurer" while the first restore was still running. The dialog's buttons are now disabled the moment you confirm, and the screen only recreates once the whole restore has genuinely finished, so it closes exactly once, for good.
